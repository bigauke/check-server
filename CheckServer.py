...
class Server():
    def __init__(self, name, port, connection, priotity):
        self.name = name
        self.port = port
        self.connection = connection.lower()
        self.priority = priotity.lower()
    ...
    def check_connection(self):
        ...
        try:
            if self.connection == "plain":
                ...
                ssl.SSLContext.wrap_socket(socket.create_connection((self.name, self.port), timeout=10))
                msg = f"{self.name} is up. On port {self.port} with {self.connection}"
                success = True
                self.alert = False
            else:
    ...
    def ping(self):
        try:
            output = subprocess.check_output("ping -{} 1 {}". format('n' if plataform.system().lower(
...
if __name__ == "__main__":
    try:
        ...
        servers = [
            ...
            Server("192.168.0.1", 80, "ping", "high"),
